...
>考虑到 txv.core.js 和 txv.login.js 涉及到的业务方较多，历史包袱重，直接对 cdn 链接进行覆盖可能会造成线上问题，且回滚、发布起来比较麻烦。因此决定发布新的 cdn 链接，供各业务方

v站登录组件灰度三阶段  
1. 老txv.core.js 兼容phone登录态的灰度，确保不影响线上功能。  
  
2. 灰度含有新登录组件的老txv.core.js，关闭手机号登录入口，只含有qq，微信，保证新登录组件正常登录没问题。  
  
3. 推进各团队更新新登录sdk，也不开放手机号入口，后续等产品节奏灰度放开手机号能力。
